...
export default {
  ...
  methods: {
    Get_Item() {
      Stores_Project_Categories().Show(this.id).then(response => {
    ...
    Edit_Item(){
      ...
      Stores_Project_Categories().Edit(this.items).then(response => {
...
<template>
  <div class="row">
    <div class="col-xs-12 col-sm-12 col-md-6 q-pa-xs">
      <q-input :error="this.Methods_Validation_Check(errors,'name')" outlined v-model="items.name"  type="text" label="نام دسته بندی">
